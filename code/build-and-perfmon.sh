...
python3 -c 'print("ABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789 *!?,." * 100, end="")' >input.txt
...
CMD_OPS="-o decode-fft.json -C 2 -g HPC -m $BIN_DIR/dtmf_encdec-fft decode $SCRIPT_DIR/output.wav"
CMD="$CMD_NAME $CMD_OPS"
$CMD
pretty_print decode-fft.json

echo "Running decode benchmark with Goertzel..."
CMD_OPS="-o decode-goe.json -C 2 -g HPC -m $BIN_DIR/dtmf_encdec-goertzel decode $SCRIPT_DIR/output.wav"
CMD="$CMD_NAME $CMD_OPS"
$CMD
